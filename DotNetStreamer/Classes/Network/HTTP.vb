...
#Region "Dictionaries"
        ...
        Private Shared ReadOnly MIMETypes As New Dictionary(Of String, String)() From { _
...
#Region "Properties"
        Private _TimeOut As Integer = 60000
        ...
        Public Property AllowMetaRedirect As Boolean
            Get
                Return _AllowMetaRedirect
            End Get
            Set(ByVal value As Boolean)
                _AllowMetaRedirect = value
            End Set
        End Property
#End Region
...
#Region "Miscellaneous"
        Public Shared Function CountOccurance(ByVal Data As String, ByVal Search As String, Optional ByVal CaseSensitive As Boolean = False) As Integer
            Return Convert.ToInt32(Convert.ToInt32(IIf(CaseSensitive, (Data.Length - (Data.Replace(Search, "").Length)), (Data.Length - (Data.ToLower.Replace(Search.ToLower, "").Length)))) / Search.Length)
        End Function
        Public Shared Function FixData(ByVal Data As String) As String
            ' Ghetto hack
            Return HtmlDecode(Data).Replace("\u003e", ">").Replace("\u003c", "<").Replace("\u003a", ":").Replace("\u003b", ";").Replace("\u003f", "?").Replace("\u003d", "=").Replace("\u002f", "/").Replace("\u0026", "&").Replace("\u002b", "+").Replace("\u0025", "%").Replace("\u0027", "'").Replace("\u007b", "{").Replace("\u007d", "}").Replace("\u007c", "|").Replace("\u0022", """").Replace("\u0023", "#").Replace("\u0021", "!").Replace("\u0024", "$").Replace("\u0040", "@").Replace("\002f", "/").Replace("\r\n", vbCrLf & vbCrLf).Replace("\n", vbCrLf).Replace("\x3a", ":").Replace("\x2f", "/").Replace("\x3f", "?").Replace("\x3d", "=").Replace("\x26", "&").Replace("\&", "&").Replace("\/\/", "//").Replace("\\", "\").Replace("\""", """").Replace("\/", "/")
        End Function
...
#Region "Parsing"
        Public Shared Function ParseMetaRefreshUrl(ByVal Html As String) As String
            ...
            Dim Find As String = Html.ToLower
            Dim Result As String = Html.Substring(Find.IndexOf("<meta http-equiv=""refresh""") + "<meta http-equiv=""refresh""".Length)
            Result = ParseBetween(Result, "url=", """", "url=".Length).Trim
        ...
        Public Shared Function ParseBetweenAll(ByVal Html As String, ByVal Before As String, ByVal After As String) As String()
            ' Author: [Eprouvez] (http://goo.gl/SLkaV)
            Dim Results As New List(Of String)
            Dim T As New List(Of String)
            With T
                .AddRange(Regex.Split(Html, Before))
                .RemoveAt(0)
...
#Region "Base36"
        Public Shared Function Base36Encode(ByVal Data As Int64) As String
            Dim chArray As Char() = Base36.ToCharArray
            Dim stack As New Stack(Of Char)
            Do While Not Data.Equals(0)
                stack.Push(chArray(CInt(Convert.ToInt64(Data Mod &H24))))
                Data = Convert.ToInt64(Data / &H24)
            Loop
            Return New String(stack.ToArray)
        End Function
        Public Shared Function Base36Decode(ByVal Data As String) As Int64
            Dim Result As Int64 = 0
            Dim Position As Integer = 0
            For Each c As Char In StrReverse(Data.ToLower())
                Result += (Base36.IndexOf(c) * Convert.ToInt64(Math.Pow(36, Position)))
                Position += 1
            Next
            Return Result
        End Function
...
#Region "Html"
        ...
        Public Function TrimHtml(ByVal Data As String) As String
            Return IIf(String.IsNullOrEmpty(Data), String.Empty, Regex.Replace(Data, "<.*?>", "")).ToString
        End Function
#End Region
#Region "Url"
        ...
        Public Shared Function UrlEncode2(ByVal Value As String) As String
            ...
            Return sb.ToString()
        End Function
        Public Shared Function UrlDecode(ByVal Data As String) As String
...
#Region "Uri/IP"
        ...
        Public Shared Function IsValidUri(ByVal Url As String) As Boolean
            ' Old pattern: "^(http|https)\://[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(:[a-zA-Z0-9]*)?/?([a-zA-Z0-9\-\._\?\,\'/\\\+&amp;%\$#\=~])*[^\.\,\)\(\s]$"
...
#Region "Timestamp"
        ...
        End Function
        Public Shared Function GetTimeStamp(ByVal Value As Date) As Int64
...
request:
            Try
                    ...
                    Else
                        ...
                        If Not IsNothing(.Headers(HttpResponseHeader.ContentType)) Then
                            If .Headers(HttpResponseHeader.ContentType).StartsWith("image") Then
                                ...
                                Exit Try
                            ElseIf .Headers(HttpResponseHeader.ContentType).StartsWith("video") Then

                                'Using sr As New StreamReader(Response.GetResponseStream)
                                '    hr.Bytes = Encoding.Default.GetBytes(sr.ReadToEnd)
                                '    sr.Close()
                                'End Using
                ...
                If hr.Html.ToLower.Contains("<meta http-equiv=""refresh") Or _
                    hr.Html.ToLower.Contains("window.parent.location.href =""") Or _
                    hr.Html.ToLower.Contains("window.location.replace(""") Then
                    Dim Redirect As String = String.Empty
                    If hr.Html.ToLower.Contains("<meta http-equiv=""refresh") Then
                        Redirect = GetRedirectUrl(hr.RequestUri, ParseMetaRefreshUrl(hr.Html))
                    ElseIf hr.Html.ToLower.Contains("window.parent.location.href =""") Then
                        Redirect = GetRedirectUrl(hr.RequestUri, ParseBetween(hr.Html, "window.parent.location.href=""", """", "window.parent.location.href=""".Length).Trim)
                    ElseIf hr.Html.ToLower.Contains("window.location.replace(""") Then
                        Redirect = GetRedirectUrl(hr.RequestUri, ParseBetween(hr.Html, "window.location.replace(""", """", "window.location.replace(""".Length).Trim)
                    End If

                    If AllowMetaRedirect Then
                        If Not String.IsNullOrEmpty(Redirect) Then
                            If Not IsBlackListed(Redirect) Then
                                Uri = Redirect
                                Method = Verb.GET
            ...
            Catch we As WebException
                ...
                If Not IsNothing(we.Response) Then
                    hr.WebResponse = DirectCast(we.Response, HttpWebResponse)
                    hr.StatusCode = hr.WebResponse.StatusCode
                    hr.Html = ProcessResponse(hr.WebResponse)
                End If
            Catch ex As Exception
                e = ex
            Finally
                If Not IsNothing(e) Then hr.[Error] = ProcessException(e)
                If Not IsNothing(Response) Then Response.Close()
                If hr.StatusCode.Equals(HttpStatusCode.RequestTimeout Or HttpStatusCode.GatewayTimeout) Then hr.TimedOut = True
        ...
        Public Overloads Function GetResponse(ByVal Method As Verb, ByVal Uri As String, ByVal ExtraHeaders As NameValueCollection, ByVal Fields As NameValueCollection, ByVal ParamArray Upload As UploadData()) As HttpResponse
            ...
            With Writer
                ...
                If Not IsNothing(Upload) Then
                    For Each u As UploadData In Upload
                        ...
                        .Write(("Content-Type: " & u.ContentType & vbCrLf) & vbCrLf)
                        .Flush()
            ...
            Return Result
        End Function
        Public Overloads Function GetResponse(ByVal Method As Verb, ByVal Uri As String, ByVal ContType As String, ByVal Boundary As String, ByVal ExtraHeaders As NameValueCollection, ByVal Fields As NameValueCollection, ByVal ParamArray Upload As UploadData()) As HttpResponse
            ...
            Me.ContentType = ContType
            Dim Result As HttpResponse = Me.GetResponse(Method, Uri, PostData.ToArray, ExtraHeaders)
        ...
        Public Function DownloadFile(ByVal Path As String, ByVal Url As String) As HttpResponse
            ...
            Try
                With wc
                    If Not String.IsNullOrEmpty(Referer) Then .Headers.Add(HttpRequestHeader.Referer, Referer)
                    If SendCookies Then
                        Dim Cookie As String = IIf(UseCustomCookies, GetCookies(CustomCookies), GetCookies(Url)).ToString
                        If Not String.IsNullOrEmpty(Cookie) Then .Headers.Add(HttpRequestHeader.Cookie, Cookie)
                    End If
        ...
        Public Function GetCookies(ByVal Names() As String) As HttpCookie()
            Dim Result As New List(Of HttpCookie)
            For Each Name As String In Names
                Dim c As HttpCookie = FindCookie(Name)
                If Not IsNothing(c) Then Result.Add(c)
            Next
        ...
        Public Function GetCookies(ByVal Names() As String, ByVal Domain As String) As HttpCookie()
            Dim Result As New List(Of HttpCookie)
            For Each Name As String In Names
                Dim c As HttpCookie = FindCookie(Name, Domain)
                If Not IsNothing(c) Then Result.Add(c)
            Next
            Return Result.ToArray
        End Function
...
#Region "Private Methods"
        Private Function SendRequest(ByVal Method As Verb, ByVal Uri As String, ByVal PostData() As Byte, ByVal ExtraHeaders As NameValueCollection) As HttpWebRequest
            ...
            With Request
                ...
                With hc
                    ...
                    '    Else
                    '        .Add(If(IsNothing(Proxy), "Connection", "Proxy-Connection"), "keep-alive")
                    '    End If
                    'End If

                    If SendCookies Then
                        Dim Cookie As String
                        Cookie = If(UseCustomCookies, GetCookies(CustomCookies), GetCookies(Uri))
                        If Not String.IsNullOrEmpty(Cookie) Then .Add("Cookie", HttpUtility.UrlEncode(Cookie))
                    End If
                End With
        ...
        Private Function ProcessException(ByVal Ex As Object) As HttpError
            ...
            If TypeOf Ex Is WebException Then
                ...
                If Not IsNothing(we.Response) AndAlso DirectCast(we.Response, HttpWebResponse).StatusCode.Equals(HttpStatusCode.BadGateway) Then
                    Message = we.Message
                ...
                Else
                    Select Case we.Status
                        ...
                        Case WebExceptionStatus.ConnectFailure
                            If we.Message.Trim.Equals("Unable to connect to the remote server") Then
                                Message = IIf(Not String.IsNullOrEmpty(Me.Proxy.Server), "Could not connect to proxy server.", "Could not connect to server.").ToString
                                If Not String.IsNullOrEmpty(Me.Proxy.Server) Then Result.IsProxyError = True
                            Else
                                Message = we.Message
                            End If
                        Case WebExceptionStatus.KeepAliveFailure
                            Message = IIf(Not String.IsNullOrEmpty(Me.Proxy.Server), "Disconnected from proxy server.", we.Message).ToString
                            If Not String.IsNullOrEmpty(Me.Proxy.Server) Then Result.IsProxyError = True
                        Case Else
                            Message = we.Message
            ...
            If Not IsNothing(Cookies) Then
                With Cookies
                    ...
                    For Each item As HttpCookie In Cookies
                        Result &= item.Name & IIf(String.IsNullOrEmpty(item.Value), "", "=" & item.Value).ToString & "; "
                    Next
...
nextItem:
        ...
        Private Sub ProcessCookies(ByVal Response As HttpWebResponse)
            Try
                ...
                Else
                    For Each c As String In Data.Split(Convert.ToChar(","))
        ...
        Private Sub ParseCookie(ByVal Data As String, ByVal Uri As Uri)
            ...
            With Cookie
                .Name = Data.Split(Convert.ToChar("="))(0).Trim
                If CookieBlacklist.Contains(.Name) Then Return
                ...
                If Data.Contains(";") Then
                    ...
                    .Value = .Value.Split(Convert.ToChar("="))(1).Trim
                Else
                    .Value = Data.Split(Convert.ToChar("="))(1).Trim
                End If
                If .Value.ToLower.Equals("deleted") Then
                    Dim Find As HttpCookie = FindCookie(.Name)
                    If Not IsNothing(Find) Then RemoveCookie(Find)
                Else
                    For Each Parameter As String In Split(Data, ";")
                        Parameter = Parameter.Trim
                        If Not String.IsNullOrEmpty(Parameter) Then
                            If Parameter.Contains("=") Then
                                Dim Key As String = Parameter.Split(Convert.ToChar("="))(0).Trim
                                ...
                                Select Case Key.ToLower
                                    ...
                                    Case "expires"
                                        Value = TrimDay(Value)
                                        If Value.ToLower.EndsWith("utc") Or
                                            Value.ToLower.EndsWith("gmt") Then Value = Value.Substring(0, Value.ToLower.IndexOf(IIf(Value.ToLower.EndsWith("utc"), "utc", "gmt").ToString)).Trim
                                        Try
        ...
        Private Function GetResponseHeaders(ByVal Response As HttpWebResponse) As String
            Return String.Format("Response Headers -----------------------------------{0}{1}{0}{2}", vbCrLf, "StatusCode: " & CInt(Response.StatusCode) & " " & Response.StatusDescription, Response.Headers.ToString)
        End Function
            ...
            Else
                If Redirect.StartsWith("&") Or Redirect.StartsWith("?") Or RequestUri.EndsWith("/") Then
                    If RequestUri.EndsWith("/") AndAlso Redirect.StartsWith("/") Then
                        Result = RequestUri & Redirect.Substring(1)
                    Else
                ...
                ElseIf Redirect.StartsWith("/") Then
                    ...
                    For Each Part As String In Split(Redirect, "/")
                        ...
                        End If
                    Next
                    If Not Built Then Result = IIf(RequestUri.StartsWith("https"), "https://", "http://").ToString & New Uri(RequestUri).Host & Redirect
                Else
        ...
        Private Function TrimDay(ByVal Value As String) As String
            Dim Days() As String = New String() {"Mon", "Monday", "Tue", "Tues", "Tuesday", "Wed", "Wednesday", "Thu", "Thur", "Thurs", "Thursday", "Fri", "Friday", "Sat", "Saturday", "Sun", "Sunday"}
            For Each Day As String In Days
                If Value.ToLower.Contains(Day.ToLower) Then Value = Value.ToLower.Replace(Day.ToLower, String.Empty).Trim
            Next
            Return Value
        End Function
#End Region
